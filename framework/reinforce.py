...
class REINFORCEPolicy(BasePolicy):
    ...
    def _forward(self, obs_norm: np.ndarray) -> tuple[np.ndarray, list[np.ndarray], list[np.ndarray]]:
    ...
    def on_episode_end(self) -> None:
        ...
        if self._baseline_type == "running_mean":
            self._baseline_val = (1 - self._baseline_alpha) * self._baseline_val + self._baseline_alpha * float(G[0])
    ...
    def from_cfg(
        ...
        obj = cls(
            ...
            output_dim=cfg.get("output_dim", np.array(cfg["biases"][-1]).shape[0] if "biases" in cfg else 1),
    ...
    def load_trainer_state(self, path: str) -> None:
        ...
        logger.info("[REINFORCEPolicy] trainer state loaded from %s (baseline=%.4f)", path, self._baseline_val)
...
class TwoHeadREINFORCEPolicy(BasePolicy):
    ...
    * **spatial_head** — *n_spatial* logits, sigmoid → continuous coordinates
    ...
    def _trunk_forward(self, obs_norm: np.ndarray) -> tuple[np.ndarray, list[np.ndarray], list[np.ndarray]]:
        x: np.ndarray = obs_norm.astype(np.float32)
        layer_inputs: list = []
        pre_relu: list = []
    ...
    def on_episode_end(self) -> None:
        ...
        for t in range(T):
            ...
            if self._entropy_coeff > 0.0:
                ...
                ent_grad_fn[fn_mask] = -(fn_probs[fn_mask].astype(np.float64) * (log_p_fn[fn_mask] + H_fn))
            ...
            if self._trunk_w:
                g_trunk = self._fn_w.T.astype(np.float64) @ delta_fn + self._sp_w.T.astype(np.float64) @ delta_sp
    ...
    def save_trainer_state(self, path: str) -> None:
        np.savez(path, baseline_val=np.float64(self._baseline_val), obs_dim=np.int64(self._obs_dim))
