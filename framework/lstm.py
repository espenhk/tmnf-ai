...
class LSTMCore(BasePolicy):
    ...
    def with_flat(self, flat: np.ndarray) -> "LSTMCore":
        ...
        if flat.shape[0] != self.flat_dim:
            raise ValueError(f"LSTMCore.with_flat: expected flat vector of size {self.flat_dim}, got {flat.shape[0]}")
...
class LSTMEvolutionPolicy(BasePolicy):
    ...
    def __init__(
        ...
        *,
        ...
        self._template: _LSTMIndividual = (
            _template if _template is not None else LSTMCore(obs_spec=obs_spec, hidden_size=hidden_size)
        )
    ...
    def initialize_from_champion(self, champion: _LSTMIndividual) -> None:
        ...
        if champion_flat_dim != self._flat_dim:
            mismatch_reasons.append(f"flat_dim mismatch (expected {self._flat_dim}, got {champion_flat_dim})")
        if (
            ...
            mismatch_reasons.append(
                f"hidden_size mismatch (expected {expected_hidden_size}, got {champion_hidden_size})"
            )
        if expected_obs_dim is not None and champion_obs_dim is not None and champion_obs_dim != expected_obs_dim:
            mismatch_reasons.append(f"obs_dim mismatch (expected {expected_obs_dim}, got {champion_obs_dim})")
        if mismatch_reasons:
            raise ValueError(
                "Cannot initialise LSTMEvolutionPolicy from an incompatible champion: " + "; ".join(mismatch_reasons)
            )
    ...
    def load_trainer_state(self, path: str) -> None:
        ...
        logger.info("[LSTMEvolutionPolicy] trainer state loaded from %s (sigma=%.4f)", path, self._sigma)
