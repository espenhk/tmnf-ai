"""Generic Deep Q-Network (DQN) policy.

DQNPolicy implements DQN over any discrete action set, parameterised by an
ObsSpec and a discrete_actions array.  It mirrors the mathematics of
games/tmnf/policies.py:NeuralDQNPolicy but is fully game-agnostic.

The optional *available_actions_fn* parameter absorbs the masking behaviour of
SC2NeuralDQNPolicy (games/sc2/policies.py:568): when supplied it is called with
the latest ``info`` dict to produce a boolean mask over *discrete_actions* that
is applied before argmax / random exploration.
"""
...
class DQNPolicy(BasePolicy):
    """DQN over an arbitrary discrete action set.

    Online network:  Q(s, a; θ_online)
    Target network:  Q(s, a; θ_target)  — synced every *target_update_freq* steps
    Replay buffer:   circular buffer of (s, a_idx, r, s', done)

    Architecture: obs → Linear → ReLU → … → Linear(n_actions)
    Pure numpy; Adam optimiser; ε-greedy exploration with linear ε decay.
    ...
    discrete_actions :
        Array of shape ``(n_actions, action_dim)``; each row is the action
        vector returned for the corresponding Q-index.
    available_actions_fn :
        Optional callable ``(info: dict) -> np.ndarray[bool]`` of shape
        ``(n_actions,)``.  When set, illegal actions receive ``-inf`` Q-value
        during greedy selection and random exploration is restricted to legal
        actions.  The internal buffer is switched to
        :class:`~framework.replay.MaskedReplayBuffer` so that the target
    ...
    def __init__(
        ...
        *,
        ...
        self._discrete_actions = np.asarray(discrete_actions)
        self._n_actions        = len(self._discrete_actions)
        ...
        self._eps_delta   = (
            ...
            / max(1, int(epsilon_decay_steps))
        )
        self._gamma  = float(gamma)
        self._seed   = seed

        if self._avail_fn is not None:
            self._replay: ReplayBuffer = MaskedReplayBuffer(replay_buffer_size)
        else:
    ...
    def from_cfg(
        ...
        if "online_weights" in cfg:
            ...
            logger.info(
                "[DQNPolicy] loaded weights from cfg (eps=%.4f, steps=%d)",
    ...
    def _build_net(self) -> dict:
        """He-initialised MLP: obs_dim → hidden… → n_actions."""
    ...
    def __call__(self, obs: np.ndarray) -> np.ndarray:
        ...
        if np.random.random() < self._eps:
            if mask is not None:
                ...
                idx   = int(np.random.choice(avail))
            else:
                idx = np.random.randint(self._n_actions)
        else:
            q = self._q_values(self._online, obs_norm)
            if mask is not None:
                q_m = q.copy().astype(np.float64)
                q_m[~mask] = -np.inf
                idx = int(np.argmax(q_m))
            else:
                idx = int(np.argmax(q))

        return self._discrete_actions[idx].copy()

    def _action_to_idx(self, action: np.ndarray) -> int:
        """Find the index of *action* in discrete_actions (linear scan)."""
        for i, a in enumerate(self._discrete_actions):
            if np.allclose(action, a):
                return i
        return 0
    ...
    def update(
        ...
        **kwargs,
        ...
        action_idx = int(action) if np.isscalar(action) else self._action_to_idx(action)
        mask = self._get_available_mask()

        if isinstance(self._replay, MaskedReplayBuffer):
            available_mask = (
                mask if mask is not None
                else np.ones(self._n_actions, dtype=bool)
            )
            self._replay.add(obs, action_idx, reward, next_obs, done, available_mask)
        else:
            self._replay.add(obs, action_idx, reward, next_obs, done)
        ...
        if len(self._replay) >= self._min_replay:
            ...
            if isinstance(self._replay, MaskedReplayBuffer):
                obs_b, act_b, rew_b, next_b, done_b, mask_b = batch
    ...
    def on_episode_end(self) -> None:
        pass  # ε decays per step, not per episode
    ...
    def load_trainer_state(self, path: str) -> None:
        ...
        network architecture (e.g. obs_spec or hidden_sizes changed).
        """
