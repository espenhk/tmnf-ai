...
class SC2Client:
    ...
    def _action_to_call(self, action: np.ndarray) -> Any:
        ...
        if self._available_actions is not None and int(fn_call.function) not in self._available_actions:
            ...
            if selection_required and no_units_selected and select_army_id in self._available_actions:
                ...
                if (
                    ...
                    logger.debug(
                        "Action %s blocked; auto-selecting army (#124).",
                        fn_name,
                    )
                ...
                logger.debug(
                    "Action %s still blocked after select_army; issuing no_op.",
                    fn_name,
                )
            else:
                self._blocked_unit_targeted_steps = 0
