...
def _make_genetic(pop=6, elite=2, eval_episodes=1, obs_spec=None) -> SC2GeneticPolicy:
    spec = obs_spec or SC2_MINIGAME_OBS_SPEC
...
class TestSC2MultiHeadLinearPolicyInit(unittest.TestCase):
    ...
    def test_fn_weights_shape_ladder(self):
        ...
        self.assertEqual(p._fn_weights.shape, (N_FUNCTION_IDS, SC2_LADDER_OBS_SPEC.dim))

    def test_spatial_weights_shape_ladder(self):
        p = _make_policy(SC2_LADDER_OBS_SPEC)
        self.assertEqual(p._sp_weights.shape, (N_SPATIAL_ROWS, SC2_LADDER_OBS_SPEC.dim))
...
class TestSC2MultiHeadLinearPolicySerialization(unittest.TestCase):
    ...
    def test_from_cfg_ignores_legacy_spatial_keys(self):
        ...
        names = SC2_MINIGAME_OBS_SPEC.names
        legacy_cfg = {f"spatial_{i}_weights": {n: 1.0 for n in names} for i in range(9)}
...
class TestSC2GeneticPolicyCrossover(unittest.TestCase):
    ...
    def test_crossover_draws_from_both_parents(self):
        cfg1, cfg2 = self._two_extreme_cfgs()
        child_cfg = SC2GeneticPolicy._crossover(cfg1, cfg2)
        flat_child = SC2MultiHeadLinearPolicy.from_cfg(child_cfg, SC2_MINIGAME_OBS_SPEC).to_flat()
...
class TestSC2GeneticPolicySerialization(unittest.TestCase):
    ...
    def test_champion_yaml_round_trips_losslessly(self):
        ...
        try:
            ...
            os.unlink(path)
        np.testing.assert_array_almost_equal(gp._champion._fn_weights, loaded._fn_weights, decimal=6)
        np.testing.assert_array_almost_equal(gp._champion._sp_weights, loaded._sp_weights, decimal=6)
    ...
    def test_from_cfg_restores_champion_weights(self):
        ...
        np.testing.assert_array_almost_equal(gp1._champion._fn_weights, gp2._champion._fn_weights, decimal=6)
        np.testing.assert_array_almost_equal(gp1._champion._sp_weights, gp2._champion._sp_weights, decimal=6)
...
class TestSC2MultiHeadLinearPolicyAvailableMask(unittest.TestCase):
    ...
    def test_mask_applied_after_update(self):
        ...
        p.update(dummy, dummy, 0.0, dummy, False, info={"available_fn_ids": {0, 1, 2}})
