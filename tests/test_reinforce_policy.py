...
class TestREINFORCEDiscountedReturns(unittest.TestCase):
    def _run_synthetic(self, rewards):
        policy = _make(hidden_sizes=[8], learning_rate=0.1, gamma=0.9, entropy_coeff=0.0, seed=42)
        obs = _zero_obs()
    ...
    def test_weights_change_after_update(self):
        policy = _make(hidden_sizes=[8], learning_rate=0.5, gamma=0.99, entropy_coeff=0.0, seed=1)
        ...
        for i, (wb, wa) in enumerate(zip(w_before, policy._weights)):
            self.assertFalse(np.allclose(wb, wa), f"weights[{i}] unchanged after gradient step")
    ...
    def test_gradient_direction(self):
        ...
        """
        np.random.seed(123)
        policy = _make(hidden_sizes=[16], learning_rate=0.05, gamma=1.0, entropy_coeff=0.0, baseline="none", seed=0)
        ...
        self.assertGreater(diff_after, diff_before, "REINFORCE did not increase probability of rewarded action")
...
class TestREINFORCEEntropyCoeff(unittest.TestCase):
    def test_zero_entropy_coeff_no_entropy_term(self):
        ...
        policy.on_episode_end()
        changed = any(not np.allclose(w_ref[i], policy._weights[i]) for i in range(len(policy._weights)))
    ...
    def test_entropy_coeff_changes_gradient(self):
        ...
        def _weights_after(entropy_coeff):
            p = _make(hidden_sizes=[8], learning_rate=0.5, gamma=1.0, entropy_coeff=entropy_coeff, seed=77)
            p(obs)
...
class TestREINFORCECfgRoundtrip(unittest.TestCase):
    ...
    def test_from_cfg_restores_hyperparams(self):
        policy = _make(hidden_sizes=[12], learning_rate=0.005, gamma=0.95, entropy_coeff=0.02, baseline="none")
        cfg = policy.to_cfg()
        loaded = REINFORCEPolicy.from_cfg(cfg, _OBS_SPEC, _ACTION_DEC)
...
class TestREINFORCETrainerState(unittest.TestCase):
    def test_save_load_roundtrip_baseline(self):
        ...
        self.assertNotEqual(policy._baseline_val, 0.0, "Expected baseline_val to change after training episodes")
